...
function Registro({ rut }) {
  ...
  const modificarCita = async (id, nuevaFecha, nuevaHora, nuevoRut, nuevoExamen, nuevoMotivo, nuevoIdMedico, nuevoEquipo, rut) => {
    try {
      ...
      const url = `http://localhost:5000/Citas/${id}`;  // Agregar el id a la URL
  ...
  const handleBuscar1 = async () => {
    try {
      if ( 
        ...
        const response = await fetch(`http://localhost:5000/Citas/${busqueda.rut}/${busqueda.fecha}/${busqueda.hora}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
          }
